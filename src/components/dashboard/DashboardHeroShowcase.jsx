...
export const DashboardHeroShowcase = () => {
  ...
  return (
    <div className="relative w-full h-[320px] rounded-3xl overflow-hidden border border-slate-800 shadow-2xl group bg-slate-950">
      {/* Background Slides */}
      {DASHBOARD_BANNERS.map((banner, index) => (
        <div
          key={banner}
          className={`absolute inset-0 bg-cover bg-center transition-all duration-1000 ease-in-out ${
            index === currentIndex ? 'opacity-40 scale-100' : 'opacity-0 scale-105'
          }`}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#0b0f19] via-[#0b0f19]/35 to-transparent pointer-events-none" />
      <div className="absolute inset-0 bg-gradient-to-r from-[#0b0f19]/80 via-transparent to-transparent pointer-events-none" />

      {/* Content overlay */}
      <div className="absolute bottom-0 left-0 p-8 z-10 space-y-2 max-w-lg">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 uppercase tracking-widest">
