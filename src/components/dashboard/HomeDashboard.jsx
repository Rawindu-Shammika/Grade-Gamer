...
export const HomeDashboard = ({ session, logout }) => {
  const [activeSelectedGame, setActiveSelectedGame] = useState('Valorant');
  ...
  const cardClass = 'bg-[#121620] border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4';
  const labelClass = 'block text-[8px] font-mono font-bold uppercase tracking-widest text-slate-500';
  const valClass = 'text-xs font-black text-white uppercase tracking-wide';
  ...
  return (
    <div className="bg-[#0b0f19] text-slate-100 font-sans relative z-10 w-full max-w-7xl mx-auto px-6 pt-24 pb-12 space-y-28 flex-grow">
        ...
        <div className={cardClass}>
          ...
          <div className="space-y-3 pt-1">
            ...
            <div>
              ...
              <span className="text-xs font-black text-white font-mono">{userEmail}</span>
            </div>
            <div>
              <span className={labelClass}>System UID</span>
              <span className="text-[10px] font-mono text-slate-400 break-all">{userId}</span>
            </div>
          </div>
        </div>
      ...
      <div className="pt-4 space-y-6">
        ...
        {/* designated 7 games grid slider */}
        <ActiveGamesGrid
          activeSelectedGame={activeSelectedGame}
          setActiveSelectedGame={setActiveSelectedGame}
          currentLccDelta={lccDelta}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-[#121620] border border-slate-800 p-5 rounded-xl shadow-xl space-y-4">
              <div className="space-y-1">
                <h4 className="text-xs font-black uppercase tracking-wider text-white">Manual Telemetry Injection</h4>
            ...
            {/* stream data log */}
            <div className="bg-[#121620] border border-slate-800 p-5 rounded-xl shadow-xl space-y-4">
              <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 border-b border-slate-800 pb-2 flex items-center gap-2 font-mono">
              ...
              <div className="max-h-[250px] overflow-y-auto space-y-2.5 pr-1 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
                {isLoading && matchesList.length === 0 ? (
                  ...
                  <div className="text-center py-8 rounded-xl border border-dashed border-slate-800 bg-slate-900/10 text-slate-500 space-y-2">
                    <Terminal className="w-5 h-5 mx-auto opacity-55" />
                    <p className="text-xs font-medium">No matches logged for {activeSelectedGame}.</p>
                    <p className="text-[9px] text-slate-500">Inject mock match data above.</p>
                  </div>
                ) : (
                  [...matchesList].reverse().map((match, idx) => {
                    ...
                    return (
                      <div
                        key={match.id || idx}
                        className="p-3 rounded-lg bg-slate-900/60 border border-slate-800/60 flex items-center justify-between hover:border-slate-800 transition-all shadow-sm"
                      >
                        <div className="space-y-0.5 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-[9px] font-mono text-slate-500">{date}</span>
                            <span className="text-[8px] font-bold px-1 py-0.5 rounded bg-slate-800 text-slate-400 uppercase tracking-wide">
                              Match #{matchesList.length - idx}
                            </span>
                          </div>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">{activeSelectedGame}</p>
                        </div>
                        <div className="flex-shrink-0 text-right">
                          <span className="text-xs font-black text-white font-mono bg-slate-950 border border-white/5 px-2.5 py-1 rounded-md">
